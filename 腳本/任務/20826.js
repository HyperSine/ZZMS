...
function start(mode, type, selection) {
    ...
    if (status == 0) {
        qm.sendNext("首先針對耶雷弗來做個介紹。耶雷弗是在女皇的魔法籠罩之下的漂浮島。所以雖然飄浮在這裡，但是卻扮演著楓之谷往返之船的角色。");
    } else if (status == 1) {
...
function end(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            qm.sendNextPrev("我還有很多話要說呢。我那驚人的活躍…！");
        } else if (status == 5) {
            qm.sendNextPrevS("那個我日後會慢慢教育的。那麼請慢走吧。", 1, 0, 1102006);
        } else if (status == 6) {
