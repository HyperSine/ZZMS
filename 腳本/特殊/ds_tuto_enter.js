...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        cm.getDirectionInfo("Effect/Direction6.img/effect/tuto/balloonMsg1/14", 2000, 0, -100, 0, 0);
        cm.getDirectionInfo(0, 326, 0);
        cm.environmentChange("demonSlayer/31111003", 5);
        cm.getDirectionInfo("Skill/3111.img/skill/31111003/effect", 0, 0, 0, 0, 0);
        cm.setNPCSpecialAction(2159309, "teleportation");
        cm.getDirectionInfo(1, 570);
    } else if (status == 5) {
        cm.removeNPCRequestController(2159309);
        cm.getDirectionInfo(1, 1200);
    } else if (status == 6) {
        cm.spawnNPCRequestController(2159309, 180, 50, 1);
        cm.setNPCSpecialAction(2159309, "summon");
        cm.getDirectionInfo(1, 360);
    } else if (status == 7) {
        cm.sendNextS("噢噢~~這樣已經足以視為是背叛的行為。 你還無法捨棄人類的心嗎？ 竟然因為區區的家人而如此墮落，真是令人心寒呀！…", 5, 2159308);
    } else if (status == 8) {
        cm.getDirectionInfo("Effect/Direction6.img/effect/tuto/balloonMsg1/15", 2000, 0, -100, 0, 0);
        cm.getDirectionInfo(3, 1);
    } else if (status == 9) {
        cm.getDirectionInfo(0, 365, 0);
