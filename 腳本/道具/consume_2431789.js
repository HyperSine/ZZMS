...
function start() {
    var menu = im.getSkillMenu(20, im.getJob());
    if (menu == "") {
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        if (selection > 0 && im.canUseSkillBook(selection, 20) && im.haveItem(2431789)) {
            im.gainItem(2431789, -1);
            im.useSkillBook(selection, 20);
        } else if (selection != 0) {
            im.sendOkS("目前想使用精通秘笈的話,你選擇的技能等級還太低.請你提升該技能到等級5以上吧.", 4, 2080009);
        }
        im.dispose();
    } else {
        im.dispose();
